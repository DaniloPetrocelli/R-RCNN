...
def draw_rectangle(drawing, coordinates, color, width=3):
    #top
...
def save_tracking_bbox(image, image_name, boxes, save_dir=None, mode=0):
    """
        image: image array
        image_name: image name
        boxes: [num_instance, (y1, x1, y2, x2)] in image coordinates.
                0: Annotation:          Red
                1: Mask RCNN result:    Green
                2: LSTM result:         Blue
        save_dir: (optional) the path to store image
        mode: (optional) select the result which you want
                mode = 0 , save image with bbox;
                mode = 1 , save bbox with black background;
    """
    mode_list = [0, 1]
    assert mode in mode_list, "mode's value should in mode_list %s" % str(mode_list)
    ...
    for i in range(boxes.shape[0]):
        ...
        draw_rectangle(drawing=draw, coordinates=[x1, y1, x2, y2], color=color, width=2)
        # draw.rectangle((x1, y1, x2, y2), outline=color)
