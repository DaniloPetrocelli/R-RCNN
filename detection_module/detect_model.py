...
class PyramidROIAlignLevel(KE.Layer):
    """Find the layer of feature map which every ROI corresponding to.
    ...
    Output:
        roi_level: [batch, num_boxes, level].
    """

    def __init__(self, pool_shape, image_shape, **kwargs):
        super(PyramidROIAlignLevel, self).__init__(**kwargs)
...
class PyramidROIAlign(KE.Layer):
    ...
    def __init__(self, pool_shape, image_shape, **kwargs):
        ...
        self.pool_shape = tuple(pool_shape) # (7, 7)
        self.image_shape = tuple(image_shape) # (640, 640, 3)
...
def refine_detections(rois, probs, deltas, window, roi_level, config):
    ...
    # Index of the whole ROIs
    roi_index = np.arange(config.POST_NMS_ROIS_INFERENCE)

    # Arrange output as [N, (y1, x1, y2, x2, class_id, score, roi_level)]
    # Coordinates are in image domain.
    result = np.hstack((refined_rois[keep],
                        class_ids[keep][..., np.newaxis],
                        class_scores[keep][..., np.newaxis],
                        roi_level[keep][..., np.newaxis],
                        roi_index[keep][..., np.newaxis]))
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        def wrapper(rois, mrcnn_class, mrcnn_bbox, image_meta, roi_level):
            detections_batch = []
            for b in range(self.config.BATCH_SIZE):
                _, _, window, _ = parse_image_meta(image_meta)
                detections = refine_detections(
                    rois[b], mrcnn_class[b], mrcnn_bbox[b], window[b], roi_level[b], self.config)
...
def fpn_classifier_graph(rois, feature_maps,
    ...
    roi_level = PyramidROIAlignLevel([pool_size, pool_size], image_shape,
                        name="roi_align_level")([rois] + feature_maps)
    ...
    return mrcnn_class_logits, mrcnn_probs, mrcnn_bbox, roi_level
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox, roi_level =\
            ...
            # Detections
            # output is [batch, num_detections, (y1, x1, y2, x2, class_id, score, roi_level, roi_index)] in image coordinates
            detections = DetectionLayer(config, name="mrcnn_detection")(
                [rpn_rois, mrcnn_class, mrcnn_bbox, input_image_meta, roi_level])
            ...
            model = KM.Model([input_image, input_image_meta], # Input
                             [detections, mrcnn_class, mrcnn_bbox, # Output
    ...
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window):
        ...
        detections: [N, (y1, x1, y2, x2, class_id, score, roi_level, roi_index)]
        ...
        final_roi_level = detections[:N, 6]
        final_roi_idx = detections[:N, 7]
        ...
        return boxes, class_ids, scores, full_masks, final_roi_level, final_roi_idx
    ...
    def detect(self, images, verbose=0):
        ...
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks, final_roi_level, final_roi_idx =\
            ...
            results.append({
                ...
                "scores": final_scores,
                "masks": final_masks,
                "roi_level": final_roi_level,
                "roi_index": final_roi_idx
            })
        return results
